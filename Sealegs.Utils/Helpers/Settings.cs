...
namespace Sealegs.Clients.Portable
{
    ...
    public class Settings : INotifyPropertyChanged
    {
        ...
        const string ShowInactiveLockersKey = "all_inactive";
        static readonly bool ShowInactiveLockersDefault = false;

        /// <summary>
        /// Gets or sets a value indicating whether the user wants to show only inactive lockers.
        /// </summary>
        /// <value><c>true</c> if show all categories; otherwise, <c>false</c>.</value>
        public bool ShowInactiveLockers
        {
            get { return AppSettings.GetValueOrDefault<bool>(ShowInactiveLockersKey, ShowInactiveLockersDefault); }
            set
            {
                if (AppSettings.AddOrUpdateValue<bool>(ShowInactiveLockersKey, value))
                    OnPropertyChanged();
            }
        }

        const string ShowLastUsedLockersKey = "show_last_used_lockers";
        static readonly bool ShowLastUsedLockersDefault = false;

        /// <summary>
        /// Gets or sets a value indicating whether the user wants to show lockers sorted by last viewed
        /// </summary>
        /// <value><c>true</c> if show past lockers; otherwise, <c>false</c>.</value>
        public bool ShowLastUsedLockers
        {
            get
            {
                return AppSettings.GetValueOrDefault<bool>(ShowLastUsedLockersKey, ShowLastUsedLockersDefault);
            }
            set
            {
                if (AppSettings.AddOrUpdateValue<bool>(ShowLastUsedLockersKey, value))
                    OnPropertyChanged();
            }
        }

        const string FilteredLockersKey = "filtered_lockers";
        static readonly string FilteredLockersDefault = string.Empty;

        public string FilteredLockers
        {
            get { return AppSettings.GetValueOrDefault<string>(FilteredLockersKey, FilteredLockersDefault); }
            set
            {
                if (AppSettings.AddOrUpdateValue<string>(FilteredLockersKey, value))
                    OnPropertyChanged();
            }
        }
        ...
        const string RoleKey = "role_key";
        readonly string RoleDefault = string.Empty;
        public string Role
        {
            get { return AppSettings.GetValueOrDefault<string>(RoleKey, RoleDefault); }
            set
            {
                if (AppSettings.AddOrUpdateValue(RoleKey, value))
                {
                    OnPropertyChanged();
                    OnPropertyChanged(nameof(UserDisplayName));
                }
        ...
        public string UserDisplayName => IsLoggedIn ? $"{FirstName} {LastName} - {Role}" : "Sign In";
