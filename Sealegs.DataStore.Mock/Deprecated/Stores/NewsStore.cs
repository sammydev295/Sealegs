...
namespace Sealegs.DataStore.Mock
{
    public class NewsStore : BaseStore<News>, INewsStore
    {
        ...
        public override async Task<News> GetItemAsync(string id)
        {
            await InitializeStore();
            return News.FirstOrDefault(s => s.Id == id);
        }
        ...
        public override async Task<IEnumerable<News>> GetItemsAsync(bool forceRefresh = false)
        {
            ...
            return News as IEnumerable<News>;
        }
        ...
        List<NewsLevel> newsLevels;
        NewsLevel GetLevel(int level)
        {
            if (newsLevels == null)
            {
                newsLevels = new List<NewsLevel> {
                    new NewsLevel { Name = "Platinum", Rank = 0 },
                    new NewsLevel { Name = "Gold", Rank = 1 },
                    new NewsLevel{ Name = "Silver", Rank = 2 },
                    new NewsLevel { Name = "Exhibitor", Rank = 3 }
                };
                var json = Newtonsoft.Json.JsonConvert.SerializeObject(newsLevels);
            }

            return newsLevels[level];
        }

        public Task<IEnumerable<News>> GetLatestNews()
        {
