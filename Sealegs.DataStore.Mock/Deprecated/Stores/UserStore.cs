...
namespace Sealegs.DataStore.Mock
{
    ...
    {
        IEnumerable<SealegsUser> users;

        #region ISpeakerStore implementation

        public override async Task<SealegsUser> GetItemAsync(string id)
        {
            if (!initialized)
        ...
        public override async Task<IEnumerable<SealegsUser>> GetItemsAsync(bool forceRefresh = false)
        {
            if (!initialized)
                await InitializeStore();
    ...
    static class SampleData
    {
        ...
        public static IEnumerable<SealegsUser> GetUsers() => Generate();

        static IEnumerable<SealegsUser> Generate()
        {
            random = new Random(0);
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                yield return new SealegsUser
                {
                    ...
                    IsAnnonymous = false,
                    IsApproved = true,
                    Role = new SealegsUserRole() { RoleName = SealegsUserRole.BartenderRole },
                    AvatarImage = photos.PhotoUrl
                };
            }
        }
